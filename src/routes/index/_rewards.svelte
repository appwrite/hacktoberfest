<section class="bg-white pt-[80px] pb[100px]" id="section-swag">
	<div class="container mx-auto px-6">
		<div class="text-center max-w-4xl mx-auto">
			<p class="text-black uppercase text-[14px] title tracking-[2px]">swag</p>
			<h1 class="title text-[32px] lg:text-[48px] font-semibold mt-[29px] mb-[40px]">
				Win our Limited Edition swag!
			</h1>

			<div class="flex justify-center mb-[80px]">
				<img class="h-[120px]" src="/icons/trophy.webp" width="245" height="120" alt="Trophy" />
			</div>

			<!-- <p class="text-black">
				Lorem ipsum dolor sit amet consectetur adipisicing elit. Error doloribus laborum quibusdam, ea facilis laudantium! Quibusdam, dolorum sint? Velit optio ea quae tempore itaque totam ipsum aperiam provident ducimus explicabo!
			</p> -->
		</div>

		<div class="grid grid-cols-12 gap-x-0 gap-y-[40px] lg:gap-12 mt-12">
			<div class="hidden lg:grid col-span-12 grid-cols-12">
				<div class="col-span-8">
					<div class="w-[calc(100%+10rem)] bg-[#20007F] aspect-w-16 aspect-h-9 rounded-xl">
						<img src="/swag/tier4.webp" class="rounded-xl object-cover" alt="First place" />
					</div>
				</div>

				<div class="col-span-4 flex items-center justify-center">
					<div
						class="relative w-full bg-white rounded-xl custom-box-shadow p-[48px] sm:p-8 text-center"
					>
						<h2 class="mt-[48px] mb-[22px] title text-[20px] font-semibold">
							4 contributions or more
						</h2>

						<p class="text-[16px] text-[#454545] leading-relaxed">
							Limited Edition Appwrite Hacktoberfest T-shirt + Hacktoberfest Sticker pack + OG
							Appwrite Sticker Pack + Mug
						</p>

						<div class="flex justify-center absolute left-0 -top-20 w-full pointer-events-none">
							<div
								class="text-[40px] title font-semibold text-black rounded-full bg-white custom-shadow w-[120px] h-[120px] flex items-center justify-center"
							>
								1
							</div>
						</div>
					</div>
				</div>
			</div>

			<div class="lg:hidden col-span-12 lg:col-span-4 flex flex-col">
				<div class="w-full bg-[#20007F] aspect-w-4 aspect-h-3 rounded-xl">
					<img src="/swag/tier4.webp" class="rounded-xl object-cover" alt="First place" />
				</div>
				<div class="relative -mt-10 px-4 sm:px-8 lg:flex-1">
					<div
						class="lg:h-full relative w-full bg-white rounded-xl custom-box-shadow p-[32px] sm:p-8 text-center"
					>
						<h2 class="mt-[32px] mb-[12px] title text-[20px] font-semibold">
							4 contributions or more
						</h2>

						<p class="text-[16px] text-[#454545] leading-relaxed">
							Limited Edition Appwrite Hacktoberfest T-shirt + Hacktoberfest Sticker pack + OG
							Appwrite Sticker Pack + Mug
						</p>

						<div class="flex justify-center absolute left-0 -top-10 w-full pointer-events-none">
							<div
								class="text-[24px] title font-semibold text-black rounded-full bg-white custom-shadow w-[80px] h-[80px] flex items-center justify-center"
							>
								1
							</div>
						</div>
					</div>
				</div>
			</div>

			<div class="col-span-12 lg:col-span-4 flex flex-col">
				<div class="w-full bg-[#20007F] aspect-w-4 aspect-h-3 rounded-xl">
					<img src="/swag/tier3.webp" class="rounded-xl object-cover" alt="Second place" />
				</div>
				<div class="relative -mt-10 px-4 sm:px-8 lg:flex-1">
					<div
						class="lg:h-full relative w-full bg-white rounded-xl custom-box-shadow  p-[32px] sm:p-8 text-center"
					>
						<h2 class="mt-[32px] mb-[12px] title text-[20px] font-semibold">3 contributions</h2>

						<p class="text-[16px] text-[#454545] leading-relaxed">
							OG Appwrite sticker pack + Limited Edition Hacktoberfest sticker pack + Mug
						</p>

						<div class="flex justify-center absolute left-0 -top-10 w-full pointer-events-none">
							<div
								class="text-[24px] title font-semibold text-black rounded-full bg-white custom-shadow w-[80px] h-[80px] flex items-center justify-center"
							>
								2
							</div>
						</div>
					</div>
				</div>
			</div>

			<div class="col-span-12 lg:col-span-4 flex flex-col">
				<div class="w-full bg-[#20007F] aspect-w-4 aspect-h-3 rounded-xl">
					<img src="/swag/tier2.webp" class="rounded-xl object-cover" alt="Third place" />
				</div>
				<div class="relative -mt-10 px-4 sm:px-8 lg:flex-1">
					<div
						class="lg:h-full relative w-full bg-white rounded-xl custom-box-shadow  p-[32px] sm:p-8 text-center"
					>
						<h2 class="mt-[32px] mb-[12px] title text-[20px] font-semibold">2 contributions</h2>

						<p class="text-[16px] text-[#454545] leading-relaxed">
							OG Appwrite sticker pack + Limited Edition Hacktoberfest sticker pack
						</p>

						<div class="flex justify-center absolute left-0 -top-10 w-full pointer-events-none">
							<div
								class="text-[24px] title font-semibold text-black rounded-full bg-white custom-shadow w-[80px] h-[80px] flex items-center justify-center"
							>
								3
							</div>
						</div>
					</div>
				</div>
			</div>

			<div class="col-span-12 lg:col-span-4 flex flex-col">
				<div class="w-full bg-[#20007F] aspect-w-4 aspect-h-3 rounded-xl">
					<img src="/swag/tier1.webp" class="rounded-xl object-cover" alt="Fourth place" />
				</div>
				<div class="relative -mt-10 px-4 sm:px-8 lg:flex-1">
					<div
						class="lg:h-full relative w-full bg-white rounded-xl custom-box-shadow  p-[32px] sm:p-8 text-center"
					>
						<h2 class="mt-[32px] mb-[12px] title text-[20px] font-semibold">1 contribution</h2>

						<p class="text-[16px] text-[#454545] leading-relaxed">OG Appwrite sticker pack</p>

						<div class="flex justify-center absolute left-0 -top-10 w-full pointer-events-none">
							<div
								class="text-[24px] title font-semibold text-black rounded-full bg-white custom-shadow w-[80px] h-[80px] flex items-center justify-center"
							>
								4
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</section>

<style>
	.custom-shadow {
		box-shadow: 15px 40px 40px 0px rgba(0, 0, 0, 0.1);
	}

	.custom-box-shadow {
		box-shadow: 15px 20px 30px 0px rgba(0, 0, 0, 0.05);
	}
</style>
